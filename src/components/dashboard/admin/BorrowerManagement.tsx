...
import { Borrower, Group, Branch, LoanOfficer, Region } from '@/types';
...
export default function BorrowerManagement({ borrowers, groups, branches, officers, regions, onUpdate }: BorrowerManagementProps) {
  ...
  const getBorrowerInfo = (borrower: Borrower) => {
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create and manage borrowers</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <SelectContent>
                    {groups.map((group) => (
                      <SelectItem key={group.id} value={group.id}>
                        {group.name} ({officers.find(o => o.id === group.loanOfficerId)?.name})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <Button type="submit" className="w-full">Create Borrower</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
      ...
      {borrowers.length === 0 && groups.length > 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-10">
            <p className="text-muted-foreground mb-4">No borrowers created yet</p>
      ...
      {groups.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-10">
            <p className="text-muted-foreground">Create groups first to add borrowers</p>
          </CardContent>
        </Card>
      )}
    </div>
