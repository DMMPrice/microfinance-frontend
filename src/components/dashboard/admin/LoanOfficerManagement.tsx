...
import { LoanOfficer, Branch, Region } from '@/types';
...
interface LoanOfficerManagementProps {
...
export default function LoanOfficerManagement({ officers, branches, regions, onUpdate }: LoanOfficerManagementProps) {
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Create and manage loan officers</p>
        </div>
                  ...
                  <SelectContent>
                    {branches.map((branch) => (
                      <SelectItem key={branch.id} value={branch.id}>
                        {branch.name} ({regions.find(r => r.id === branch.regionId)?.name})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <Button type="submit" className="w-full">Create Loan Officer</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
      ...
      {officers.length === 0 && branches.length > 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-10">
            <p className="text-muted-foreground mb-4">No loan officers created yet</p>
      ...
      {branches.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-10">
            <p className="text-muted-foreground">Create branches first to add loan officers</p>
          </CardContent>
        </Card>
      )}
    </div>
