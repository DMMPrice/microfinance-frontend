...
import {toISTNaiveISO} from "@/Helpers/dateTimeIST.js";
...
export function useEditCollectionByLedger() {
    const qc = useQueryClient();

    return useMutation({
        mutationFn: async ({ledger_id, payload, loanRef}) => {
            const ledId = String(ledger_id || "").trim();
            if (!ledId) throw new Error("ledger_id is required");

            const cleanPayload = {...(payload || {})};

            // ✅ IST conversion for backend
            if (cleanPayload.payment_date) {
                const ist = toISTNaiveISO(cleanPayload.payment_date, true);
        ...
        onSuccess: (_data, vars) => {
            // ✅ refresh Loan view (statement + schedule + summary etc.)
            invalidateLoanCommon(qc);

            // If you pass loanRef (loan_id or loan_account_no) from modal, use it:
            if (vars?.loanRef) {
                invalidateLoanDetails(qc, vars.loanRef);
            }
...
/* -------------------- DEDUCT LOAN ADVANCE -------------------- */
export function useDeductLoanAdvance() {
    const qc = useQueryClient();
    ...
    return useMutation({
        mutationFn: async ({loan_id, payload}) => {
            ...
            return (
                await apiClient.post(`/loans/${loanId}/advance/deduct`, payload)
            ).data;
        },
        onSuccess: (_data, vars) => {
            invalidateLoanCommon(qc);
