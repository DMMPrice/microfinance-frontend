...
import {useLoanMarginalMoneyBranch, useAuthUsers} from "@/hooks/useReports";
import {
    Card,
    CardContent,
    CardHeader,
...
import LoanMarginalMoneyFilters from "@/Component/Reports/LoanMarginalMoney/LoanMarginalMoneyFilters.jsx";
import LoanMarginalMoneyKpis from "@/Component/Reports/LoanMarginalMoney/LoanMarginalMoneyKpis.jsx";
import LoanMarginalMoneyTables from "@/Component/Reports/LoanMarginalMoney/LoanMarginalMoneyTables.jsx";
import {exportLoanMarginalMoneyExcel} from "@/Component/Reports/LoanMarginalMoney/loanMarginalMoney.export";
...
function buildBranchSummaryRows(data) {
    ...
    return [
        {metric: "Opening Loans With Advance", value: Number(openingBranch?.loans_with_advance || 0), type: "count"},
        {metric: "Opening Advance Balance", value: Number(openingBranch?.advance_balance_amt || 0), type: "amount"},

        {metric: "Month Advance Add Count", value: Number(closingCalc?.month_advance_add_cnt || 0), type: "count"},
        {metric: "Month Advance Add Amount", value: Number(closingCalc?.month_advance_add_amt || 0), type: "amount"},

        {
            metric: "Month Advance Deduct Count",
            value: Number(closingCalc?.month_advance_deduct_cnt || 0),
            type: "count"
        },
        {
            metric: "Month Advance Deduct Amount",
            value: Number(closingCalc?.month_advance_deduct_amt || 0),
            type: "amount"
        },

        {metric: "Closing Loans With Advance", value: Number(closingBranch?.loans_with_advance || 0), type: "count"},
        {metric: "Closing Advance Balance", value: Number(closingBranch?.advance_balance_amt || 0), type: "amount"},
...
export default function LoanMarginalMoneyPage() {
    ...
    const query = useLoanMarginalMoneyBranch({
        branchId,
        monthStart,
        monthEnd,
        persist,
    ...
    const onDownloadExcel = async () => {
        ...
        try {
            setExporting(true);
            await exportLoanMarginalMoneyExcel({
                data,
    ...
    return (
        <Card className="border-muted/60">
            <CardHeader className="space-y-1">
                <CardTitle className="text-xl">Loan Marginal Money</CardTitle>
                <CardDescription>
                    Branch-wise marginal money snapshot for selected month range
                </CardDescription>
            </CardHeader>

            <CardContent className="space-y-4">
                <LoanMarginalMoneyFilters
                    branchId={branchId}
                    setBranchId={(v) => {
                        setBranchId(v);
                        setLoad(false);
                ...
                {data && (
                    <LoanMarginalMoneyKpis
                        branchLabel={selectedBranchName || branchId}
                        data={data}
                    />
                )}

                <LoanMarginalMoneyTables
